...
class AzureIoT {
    public:
        ...
        void send_data(float l, float t, float p) {
            ...
            // while (true) {
                ...
                /*
                    {
                        "LightLevel" : 0.1277563,
                        "Temperature" : 22.983745,
                        "Pressure" : 1012.9283741
                    }
                */
                ...
                // SensorData<float, time_t> item = b.peekFromBuffer();
                // float l = item.fetchLightLevel();
                // float t = item.fetchTemperature();
                // float p = item.fetchPressure();

                sprintf(message, "{ \"LightLevel\" : %f, \"Temperature\" : %f, \"Pressure\" : %f }", l, t, p);
